...
这里有两个系数， $\lambda_{coord }$ 与 $\lambda_{noobj}$ ，前者论文中为5， 后者论文中设置为0.5， 这是因为物体检测其实是类别不均衡的问题，只有少数网格有物体，因此如果不进行权重约束，网络会倾向于约束不含有物体的格点，但我们更想要的是约束含有物体的网格预测的更准确。因此采取这样一个权重约束。
